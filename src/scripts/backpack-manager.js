...
export class BackpackManager extends Application {
    ...
    static get defaultOptions() {
        return foundry.utils.mergeObject(super.defaultOptions, {
            width: 450,
            template: `modules/${CONSTANTS.MODULE_ID}/templates/backpack-manager.hbs`,
            height: 750,
    ...
    /** @override */
    get id() {
        return `${CONSTANTS.MODULE_ID}-${this.object.backpack.uuid.replaceAll(".", "-")}`;
    }
    ...
    get stowed() {
        return this.bag.items
            .filter((item) => {
                return BackPackManagerHelpers.isValidItem(item);
            })
            .sort((a, b) => {
    ...
    async getData() {
        ...
        if (game.system.id === "dnd5e") {
            ...
            if (type === "weight") {
                ...
                const coinW = !currencyWeight
                    ? 0
                    : Object.keys(CONFIG.DND5E.currencies).reduce((acc, c) => {
                          return acc + (this.bag.system.currency[c] || 0);
                      }, 0);
                data.bagValue = this.stowed.reduce(
                    (acc, item) => {
            ...
            data.currencies = Object.keys(CONFIG.DND5E.currencies).map((key) => {
                return {
                    ...
                    max: { bag: this.bag.system.currency[key], actor: this.actor.system.currency[key] },
                };
            });
        }
    ...
    async _handleItemTransfer(sourceActor, targetActor, item, value, max) {
        ...
        const create = await BackPackManagerHelpers.setSystemSpecificValues(itemData, {
            quantity: value,
            target: targetActor,
            src: sourceActor,
        });
        ...
        if (mayDelete) {
            ...
            else await BackPackManagerHelpers.updateSystemSpecificQuantity(item, max, value);
            return true;
        }
        return false;
    }
